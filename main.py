def getProperFontSize(text: str, fontName: str, width: int):
...
def drawText(draw, location, text, text_fill, border_fill, font):
    ...
    draw.text( (x, y+BORDER_WIDTH), text, fill=border_fill, font=font)
    draw.text( (x-BORDER_WIDTH, y-BORDER_WIDTH), text, fill=border_fill, font=font)
    draw.text( (x-BORDER_WIDTH, y+BORDER_WIDTH), text, fill=border_fill, font=font)
    draw.text( (x+BORDER_WIDTH, y-BORDER_WIDTH), text, fill=border_fill, font=font)
    draw.text( (x+BORDER_WIDTH, y+BORDER_WIDTH), text, fill=border_fill, font=font)
...
memeFont = ImageFont.truetype('Impact', 32)
FONT_NAME = 'Impact'
text = "I DON'T ALWAYS REPLY"
font = ImageFont.truetype(FONT_NAME, getProperFontSize(text, FONT_NAME, WIDTH))
...
drawText(draw, (0, HEIGHT - text_height), text, COLOR_WHITE, COLOR_BLACK, font)
